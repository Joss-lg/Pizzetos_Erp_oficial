    ...
    <div class="min-h-screen flex flex-col transition-all duration-300" :class="sidebarExpanded ? 'lg:pl-60' : 'lg:pl-20'">
        ...
        <aside 
            ...
            <div class="h-24 flex px-4 border-b border-black/5 shrink-0 transition-all" :class="(sidebarExpanded || isMobile) ? 'items-center justify-between' : 'flex-col items-center justify-center gap-2 py-3'">
                ...
                {{-- Botón Toggle Escritorio (Adentro de lo amarillo) --}}
                <button @click="sidebarExpanded = !sidebarExpanded" class="hidden lg:flex p-1.5 bg-black/5 hover:bg-black/10 rounded-xl transition-colors text-slate-800 shrink-0" title="Expandir/Contraer">
            ...
            <nav class="flex-1 overflow-y-auto overflow-x-hidden p-3 space-y-1 scrollbar-hide">
                
                <p x-show="sidebarExpanded || isMobile" class="px-4 py-2 text-[10px] font-black text-black/40 uppercase tracking-[0.2em]">Operación</p>
                <div x-show="!(sidebarExpanded || isMobile)" class="h-px w-8 bg-black/10 mx-auto my-4"></div>

                <a href="{{ route('dashboard') }}" @click="if(isMobile) sidebarOpen = false" class="flex items-center px-4 py-3 rounded-xl transition-all {{ request()->routeIs('dashboard') ? 'bg-black text-amber-400 shadow-xl' : 'hover:bg-black/5 font-bold' }}" :class="(sidebarExpanded || isMobile) ? 'gap-3 justify-start' : 'justify-center'">
                    ...
                    <span x-show="sidebarExpanded || isMobile" class="text-sm font-bold uppercase italic tracking-tighter whitespace-nowrap">Inicio</span>
                </a>

                <a href="{{ route('ventas.pos') }}" @click="if(isMobile) sidebarOpen = false" class="flex items-center px-4 py-3 rounded-xl transition-all {{ request()->routeIs('ventas.pos') ? 'bg-black text-amber-400 shadow-xl' : 'hover:bg-black/5 font-bold' }}" :class="(sidebarExpanded || isMobile) ? 'gap-3 justify-start' : 'justify-center'">
                    ...
                    <span x-show="sidebarExpanded || isMobile" class="text-sm font-bold uppercase italic tracking-tighter whitespace-nowrap">Venta POS</span>
                </a>

                <a href="{{ route('ventas.pedidos') }}" @click="if(isMobile) sidebarOpen = false" class="flex items-center px-4 py-3 rounded-xl transition-all {{ request()->routeIs('ventas.pedidos') ? 'bg-black text-amber-400 shadow-xl' : 'hover:bg-black/5 font-bold' }}" :class="(sidebarExpanded || isMobile) ? 'gap-3 justify-start' : 'justify-center'">
                ...
                <a href="{{ route('gastos.index') }}" @click="if(isMobile) sidebarOpen = false" class="flex items-center px-4 py-3 rounded-xl transition-all {{ request()->routeIs('gastos.*') ? 'bg-black text-amber-400 shadow-xl' : 'hover:bg-black/5 font-bold' }}" :class="(sidebarExpanded || isMobile) ? 'gap-3 justify-start' : 'justify-center'">
                    ...
                    <span x-show="sidebarExpanded || isMobile" class="text-sm font-bold uppercase italic tracking-tighter whitespace-nowrap">Gastos</span>
                </a>

                <a href="{{ route('ventas.resume') }}" @click="if(isMobile) sidebarOpen = false" class="flex items-center px-4 py-3 rounded-xl transition-all {{ request()->routeIs('ventas.resume') ? 'bg-black text-amber-400 shadow-xl' : 'hover:bg-black/5 font-bold' }}" :class="(sidebarExpanded || isMobile) ? 'gap-3 justify-start' : 'justify-center'">
                    ...
                    <span x-show="sidebarExpanded || isMobile" class="text-sm font-bold uppercase italic tracking-tighter whitespace-nowrap">Historial</span>
                </a>

                <a href="{{ route('clientes.index') }}" @click="if(isMobile) sidebarOpen = false" class="flex items-center px-4 py-3 rounded-xl transition-all {{ request()->routeIs('clientes.*') ? 'bg-black text-amber-400 shadow-xl' : 'hover:bg-black/5 font-bold' }}" :class="(sidebarExpanded || isMobile) ? 'gap-3 justify-start' : 'justify-center'">
                    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path>
                    </svg>
                    <span x-show="sidebarExpanded || isMobile" class="text-sm font-bold uppercase italic tracking-tighter whitespace-nowrap">Clientes</span>
                </a>

                {{-- SECCIÓN DE ADMINISTRACIÓN RESTRINGIDA --}}
                @if(Auth::user()->id_ca == 1)
                    <p x-show="sidebarExpanded || isMobile" class="px-4 py-4 text-[10px] font-black text-black/40 uppercase tracking-[0.2em]">Administración</p>
                    <div x-show="!(sidebarExpanded || isMobile)" class="h-px w-8 bg-black/10 mx-auto my-4"></div>

                    <a href="{{ route('corte.index') }}" @click="if(isMobile) sidebarOpen = false" class="flex items-center px-4 py-3 rounded-xl transition-all {{ request()->routeIs('corte.*') ? 'bg-black text-amber-400 shadow-xl' : 'hover:bg-black/5 font-bold' }}" :class="(sidebarExpanded || isMobile) ? 'gap-3 justify-start' : 'justify-center'">
                        ...
                        <span x-show="sidebarExpanded || isMobile" class="text-sm font-bold uppercase italic tracking-tighter whitespace-nowrap">Corte Mensual</span>
                    </a>
                    ...
                    <div x-data="{ openCat: {{ (request()->is('recursos/categorias*') || request()->is('productos/pizzas*') || request()->is('productos/especialidades*')) ? 'true' : 'false' }} }">
                        <button @click="if(!isMobile && !sidebarExpanded) { sidebarExpanded = true; openCat = true; } else { openCat = !openCat; }" class="w-full flex items-center px-4 py-3 rounded-xl hover:bg-black/5 font-bold transition-all text-slate-900" :class="(sidebarExpanded || isMobile) ? 'justify-between' : 'justify-center'">
                            <div class="flex items-center" :class="(sidebarExpanded || isMobile) ? 'gap-3' : ''">
                                ...
                                <span x-show="sidebarExpanded || isMobile" class="text-sm font-bold uppercase italic tracking-tighter whitespace-nowrap">Categorías</span>
                            </div>
                            <svg x-show="sidebarExpanded || isMobile" :class="openCat ? 'rotate-180' : ''" class="w-4 h-4 transition-transform duration-300 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path d="M19 9l-7 7-7-7"/></svg>
                        </button>
                        <div x-show="openCat && (sidebarExpanded || isMobile)" x-cloak x-collapse class="pl-8 pr-4 space-y-1 pb-2 mt-1 whitespace-nowrap">
                            <a href="{{ route('categorias.index') }}" @click="if(isMobile) sidebarOpen = false" class="flex items-center gap-2 py-2 text-xs font-black uppercase tracking-widest hover:translate-x-1 transition-transform text-black">General</a>
                            <a href="{{ route('pizzas.index') }}" @click="if(isMobile) sidebarOpen = false" class="flex items-center gap-2 py-2 text-xs font-black uppercase tracking-widest hover:translate-x-1 transition-transform text-black">Pizzas</a>
                            <a href="{{ route('especialidades.index') }}" @click="if(isMobile) sidebarOpen = false" class="flex items-center gap-2 py-2 text-xs font-black uppercase tracking-widest hover:translate-x-1 transition-transform text-black">Especialidades</a>
                        </div>
                    </div>
                    ...
                    <div x-data="{ openProd: {{ (request()->is('productos*') && !request()->is('productos/pizzas*') && !request()->is('productos/especialidades*')) ? 'true' : 'false' }} }">
                        <button @click="if(!isMobile && !sidebarExpanded) { sidebarExpanded = true; openProd = true; } else { openProd = !openProd; }" class="w-full flex items-center px-4 py-3 rounded-xl hover:bg-black/5 font-bold transition-all text-slate-900" :class="(sidebarExpanded || isMobile) ? 'justify-between' : 'justify-center'">
                            <div class="flex items-center" :class="(sidebarExpanded || isMobile) ? 'gap-3' : ''">
                                ...
                                <span x-show="sidebarExpanded || isMobile" class="text-sm font-bold uppercase italic tracking-tighter whitespace-nowrap">Productos</span>
                            </div>
                            <svg x-show="sidebarExpanded || isMobile" :class="openProd ? 'rotate-180' : ''" class="w-4 h-4 transition-transform duration-300 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path d="M19 9l-7 7-7-7"/></svg>
                        </button>
                        
                        <div x-show="openProd && (sidebarExpanded || isMobile)" x-cloak x-collapse class="pl-8 pr-4 space-y-1 pb-2 mt-1 whitespace-nowrap">
                            <a href="{{ route('alitas.index') }}" @click="if(isMobile) sidebarOpen = false" class="flex items-center gap-2 py-2 text-xs font-black uppercase tracking-widest hover:translate-x-1 transition-transform text-black">Alitas</a>
                            <a href="{{ route('costillas.index') }}" @click="if(isMobile) sidebarOpen = false" class="flex items-center gap-2 py-2 text-xs font-black uppercase tracking-widest hover:translate-x-1 transition-transform text-black">Costillas</a>
                            <a href="{{ route('hamburguesas.index') }}" @click="if(isMobile) sidebarOpen = false" class="flex items-center gap-2 py-2 text-xs font-black uppercase tracking-widest hover:translate-x-1 transition-transform text-black">Hamburguesas</a>
                            <a href="{{ route('papas.index') }}" @click="if(isMobile) sidebarOpen = false" class="flex items-center gap-2 py-2 text-xs font-black uppercase tracking-widest hover:translate-x-1 transition-transform text-black">Papas</a>
                            <a href="{{ route('mariscos.index') }}" @click="if(isMobile) sidebarOpen = false" class="flex items-center gap-2 py-2 text-xs font-black uppercase tracking-widest hover:translate-x-1 transition-transform text-black">Mariscos</a>
                            <a href="{{ route('rectangular.index') }}" @click="if(isMobile) sidebarOpen = false" class="flex items-center gap-2 py-2 text-xs font-black uppercase tracking-widest hover:translate-x-1 transition-transform text-black">Rectangular</a>
                            <a href="{{ route('spaguetty.index') }}" @click="if(isMobile) sidebarOpen = false" class="flex items-center gap-2 py-2 text-xs font-black uppercase tracking-widest hover:translate-x-1 transition-transform text-black">Spaguetty</a>
                            <a href="{{ route('refrescos.index') }}" @click="if(isMobile) sidebarOpen = false" class="flex items-center gap-2 py-2 text-xs font-black uppercase tracking-widest hover:translate-x-1 transition-transform text-black">Refrescos</a>
                            <a href="{{ route('barra.index') }}" @click="if(isMobile) sidebarOpen = false" class="flex items-center gap-2 py-2 text-xs font-black uppercase tracking-widest hover:translate-x-1 transition-transform text-black">Barra</a>
                        </div>
                    </div>
            ...
            <div class="p-3 border-t border-black/5 shrink-0">
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit" class="w-full flex items-center justify-center px-3 py-3.5 rounded-xl bg-black text-white hover:bg-slate-800 transition-all font-black text-[10px] uppercase tracking-widest italic shadow-xl" :class="(sidebarExpanded || isMobile) ? 'gap-2' : ''">
                        ...
                        <span x-show="sidebarExpanded || isMobile">Cerrar Sesión</span>
                    </button>
                </form>
            </div>
        </aside>
